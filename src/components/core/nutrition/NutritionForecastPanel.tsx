...
export default function NutritionForecastPanel({ forecast, forecastNote }: NutritionForecastPanelProps) {
  ...
  return (
    <Card className="mt-3.5" padding="0.75rem">
      <p className="text-2xs font-black uppercase tracking-wider text-text-muted mb-2">Prognoza przy obecnym tempie</p>
        ...
        <div className="grid grid-cols-3 gap-2 mb-2">
          {([
            ...
            <div key={label} className="rounded-lg border border-border-custom/40 bg-surface-solid/30 px-2 py-1.5 text-center">
              <p className="text-2xs font-black uppercase text-text-muted">{label}</p>
              <p className="text-sm font-black text-text-primary">{w != null ? `${w}kg` : '—'}</p>
              {bf != null && <p className="text-2xs font-bold text-text-muted">{bf}% BF</p>}
            </div>
          ))}
        </div>
      )}
      ...
      {forecastNote && <p className="text-xs text-text-secondary leading-snug mt-1">{forecastNote}</p>}
    </Card>
  );
}
