...
function NutritionCard({ weeklyCalories, syncYazio, isSyncing, session }: { weeklyCalories: number; syncYazio: () => void; isSyncing: boolean; session: any }) {
    ...
    (async () => {
      try {
        const { data } = await supabase
          ...
          .limit(1)
          .maybeSingle();
        if (data?.target_kcal) { setKcalTarget(data.target_kcal); setWeeklyBudget(data.target_kcal * 7); }
  ...
  const saveManual = async () => {
    ...
    try {
      ...
      setShowManual(false);
      await fetchRows();
    } catch (e) { console.error('manual nutrition save failed', e); }
  ...
  const todayAnalysis = chart.find(r => r.key === todayRaw)?.analysis ?? chart.findLast(r => r.analysis)?.analysis ?? null;
  ...
  return (
    <section className="rounded-[24px] border border-border-custom bg-surface backdrop-blur-md p-5 shadow-sm">
      ...
      <p className="font-display text-[26px] font-black tracking-tight text-text-primary leading-none">
        ...
        <span className="ml-1 text-[12px] font-semibold text-text-muted tracking-normal">/ {weeklyBudget.toLocaleString('pl-PL')} kcal</span>
      </p>
          ...
          return (
            <div key={d.key} className="flex flex-col gap-1">
              <div className="flex h-10 flex-col justify-end">
                <div
                  className={`w-full rounded-sm transition-all duration-700 ${kcalBarColor(d.calories)} ${isToday ? 'opacity-100' : 'opacity-55'}`}
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          {todayInsulinLoad != null && (
            ...
            }`}>IL {Math.round(todayInsulinLoad)}</span>
          )}
        </div>
        <div className="flex items-baseline gap-1.5">
          <span className="text-[15px] font-black text-primary font-display">{todayProtein}g</span>
          {avgProtein7d != null && (
            <span className="text-[10px] text-text-muted">śr. {avgProtein7d}g/d</span>
          )}
        </div>
      </div>
