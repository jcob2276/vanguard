      ...
      <div className="relative w-full flex flex-col max-h-[85vh] sm:max-h-[680px] overflow-hidden">
        <div className="p-4 border-b border-border-custom/20 flex items-center justify-between shrink-0">
          <div>
            ...
            <span className="text-[10px] font-semibold text-text-muted">{d.today}</span>
          </div>
            ...
            <>
              <div className="space-y-2">
                ...
                {d.tasksList.length === 0 ? (
                  <div className="py-4 text-center text-text-muted/50 italic text-[11px]">Brak zadań w Power List na dziś.</div>
                ) : (
                  <div className="grid grid-cols-1 gap-1.5">
                    {d.tasksList.map((task) => (
                      <div key={task.idx} className={`px-3 py-2 rounded-lg border flex items-center justify-between transition-all ${task.done ? 'border-success/10 bg-success/[0.01] text-text-primary' : 'border-border-custom/40 bg-surface/30 text-text-muted'}`}>
                        <span className={`text-[11px] font-medium ${task.done ? 'line-through opacity-70' : ''}`}>{task.title}</span>
                        <span className={`text-[8px] font-bold px-1.5 py-0.5 rounded ${task.done ? 'bg-success/10 text-success' : 'bg-slate-500/10 text-slate-500'}`}>{task.done ? 'Tak' : 'Nie'}</span>
                      </div>
                    ))}
                  </div>
                )}
              ...
              <div className="space-y-1.5">
                ...
                <textarea value={d.reflectionText} onChange={(e) => d.setReflectionText(e.target.value)} placeholder="Zapisz krótkie podsumowanie lub napotkane tarcia..." rows={2} className="w-full bg-slate-50 dark:bg-white/[0.01] border border-border-custom/60 rounded-xl px-3 py-2 text-[12px] font-semibold text-text-primary placeholder:text-text-muted/30 focus:border-primary/50 outline-none transition-colors resize-none" />
              </div>

              <div className="space-y-1.5">
                <span className="text-[11px] font-bold text-text-primary block">Dodatkowe notatki z wykonania (opcjonalnie)</span>
                <textarea value={d.actualAccomplishmentText} onChange={(e) => d.setActualAccomplishmentText(e.target.value)} placeholder="Co konkretnie udało się dzisiaj dowieźć poza planem..." rows={2} className="w-full bg-slate-50 dark:bg-white/[0.01] border border-border-custom/60 rounded-xl px-3 py-2 text-[12px] font-semibold text-text-primary placeholder:text-text-muted/30 focus:border-primary/50 outline-none transition-colors resize-none" />
              </div>
