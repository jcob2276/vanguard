...
export default function GrowthMediaQueue({ links }: { links: GrowthLinkRow[] }) {
  ...
  return (
    <Card variant="glass" padding="1rem" className="h-full flex flex-col">
      ...
      {links.length === 0 ? (
        <p className="text-[12px] text-text-muted">Brak w kolejce — wrzuć linki w Keep.</p>
      ) : (
        <div className="space-y-4 overflow-y-auto max-h-[520px] pr-1 flex-1">
          {videos.length > 0 && (
            <div>
              <p className="text-[8px] font-black uppercase text-text-muted mb-1.5">Filmy / wideo</p>
          ...
          {rest.length > 0 && (
            <div>
              {videos.length > 0 && (
                <p className="text-[8px] font-black uppercase text-text-muted mb-1.5">Reszta</p>
              )}
              <ul className="space-y-1.5">
                {rest.map((row) => (
                  <MediaRow key={row.id} row={row} />
  ...
  return (
    <li className="rounded-xl border border-border-custom bg-background/40 px-3 py-2.5 flex items-start gap-2">
      ...
      <div className="flex-1 min-w-0">
        <p className="text-[11px] font-bold text-text-primary line-clamp-2">{row.title || row.domain}</p>
