          ...
          {isSaunaMode ? (
            <>
              <div className="grid grid-cols-[20px_1fr_1fr_24px] gap-2 px-0.5">
                <span />
                <span className="text-[9px] font-black uppercase tracking-widest text-text-muted text-center">Min</span>
                <span className="text-[9px] font-black uppercase tracking-widest text-text-muted text-center">°C</span>
                <span />
              </div>
                  ...
                  <div key={set.id} className="grid grid-cols-[20px_1fr_1fr_24px] gap-1.5 items-center">
                    ...
                    <div className="flex flex-col gap-1">
                      ...
                      <div className="flex gap-1 justify-center">
                        <button onClick={() => adjustWellness('reps', -5)} className="text-[9px] font-bold bg-surface active:bg-surface-solid text-text-secondary border border-border-custom hover:text-text-primary w-7 h-5 rounded flex items-center justify-center transition-colors">-5</button>
                        <button onClick={() => adjustWellness('reps', 5)} className="text-[9px] font-bold bg-surface active:bg-surface-solid text-text-secondary border border-border-custom hover:text-text-primary w-7 h-5 rounded flex items-center justify-center transition-colors">+5</button>
                      </div>
                    </div>
                    {/* Stopnie */}
                    <div className="flex flex-col gap-1">
                      <input type="number" min={0} step={1} value={set.kg}
                        onChange={e => updateSet(set.id, 'kg', e.target.value)}
                        placeholder="—" className={numInput} />
                      <div className="flex gap-1 justify-center">
                        <button onClick={() => adjustWellness('kg', -5)} className="text-[9px] font-bold bg-surface active:bg-surface-solid text-text-secondary border border-border-custom hover:text-text-primary w-7 h-5 rounded flex items-center justify-center transition-colors">-5</button>
                        <button onClick={() => adjustWellness('kg', 5)} className="text-[9px] font-bold bg-surface active:bg-surface-solid text-text-secondary border border-border-custom hover:text-text-primary w-7 h-5 rounded flex items-center justify-center transition-colors">+5</button>
                      </div>
                    </div>
