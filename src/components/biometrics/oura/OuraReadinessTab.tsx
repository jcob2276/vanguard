 ...
 * @role Zakładka Gotowość (Readiness) — Semi-circular Arc Gauge, 30d Baseline Range Tracker, Contributors.
 */
...
export function OuraReadinessTab({ oura, enhanced, strainRow }: OuraHealthHubData) {
  ...
  const getStatus = (score: number) => {
    if (score >= 85) return { title: 'Optymalny stan regeneracji 👑', subtitle: 'Twój organizm jest w doskonałej formie. Świetny dzień na wyzwania.', label: 'OPTIMAL', color: 'text-emerald-400' };
    if (score >= 70) return { title: 'Dobra gotowość do działania', subtitle: 'Wystarczający poziom energii do umiarkowanego i mocnego treningu.', label: 'GOOD', color: 'text-teal-400' };
    return { title: 'Wymagana uwaga i odpoczynek', subtitle: 'Podwyższony stres lub niedobór snu. Zadbaj o lżejszy dzień.', label: 'PAY ATTENTION', color: 'text-amber-400' };
  };
  ...
  return (
    <div className="space-y-4 text-white animate-fadeIn">
      ...
      <div className="relative overflow-hidden rounded-3xl border border-white/10 bg-slate-900/90 p-6 text-center shadow-2xl backdrop-blur-xl">
        ...
        <p className="text-3xs font-black uppercase tracking-[0.25em] text-teal-400/80">READINESS</p>
      ...
      {/* Contributors (Składniki Gotowości) */}
