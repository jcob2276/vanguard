...
import { Magnet, ShieldCheck, ShieldAlert, Zap, RefreshCw } from 'lucide-react';
...
export function OuraEmfSensorCard() {
  ...
  const total = reading?.totalMicroTesla ?? 42;
  ...
  const barColor =
    status === 'high' ? 'bg-rose-400' : status === 'moderate' ? 'bg-amber-400' : 'bg-emerald-400';
    ...
    <div className="rounded-3xl border border-white/10 bg-slate-900/90 p-5 space-y-4 shadow-xl">
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-3xs text-slate-400">
              Detekcja natężenia pola elektromagnetycznego (µT) przy głowie
            </p>
          </div>
        </div>
      ...
      <div className="p-4 rounded-2xl bg-slate-950/40 border border-white/10 space-y-3">
        ...
        <div className="flex items-baseline gap-2">
          <span className="font-display text-4xl font-black text-white">{total}</span>
          <span className="text-sm font-bold text-purple-400">µT (mikroteśli)</span>
        </div>

        {/* Progress Bar (Earth Baseline 40 µT -> 200 µT max) */}
        <div className="space-y-1">
          <div className="h-2 w-full rounded-full bg-white/10 overflow-hidden">
            <div
              className={`h-full rounded-full transition-all duration-500 ${barColor}`}
              style={{ width: `${Math.min(100, Math.max(10, (total / 200) * 100))}%` }}
            />
          </div>
      ...
      <div className="p-3.5 rounded-2xl bg-slate-950/60 border border-white/5 space-y-1.5 text-3xs text-slate-400 leading-normal">
        <div className="flex items-center gap-1.5 font-black uppercase text-purple-300">
          <Zap size={13} /> Dlaczego dystans od ładowarki ma znaczenie?
        </div>
        <p>
          Zasilacze 230V i ładowarki indukcyjne emitują zmienne pole magnetyczne, którego natężenie spada z kwadratem odległości. Odsunięcie telefonu o 50 cm od poduszki obniża EMF o ponad 90%, eliminując mikrowybudzenia.
        </p>
      </div>
