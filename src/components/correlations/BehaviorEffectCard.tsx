    ...
    <Card variant="glass" padding="1rem">
      ...
      <div className="grid grid-cols-3 gap-2 mb-3">
        <div className="rounded-xl bg-surface-solid/60 p-2 text-center">
          <p className="text-[8px] font-black uppercase text-text-muted">Z</p>
          <p className="text-[15px] font-black text-text-primary">{item.mean_with ?? '—'}</p>
          <p className="text-[8px] text-text-muted">n={item.n_with}</p>
        </div>
        <div className="rounded-xl bg-surface-solid/60 p-2 text-center">
          <p className="text-[8px] font-black uppercase text-text-muted">Bez</p>
          <p className="text-[15px] font-black text-text-primary">{item.mean_without ?? '—'}</p>
          <p className="text-[8px] text-text-muted">n={item.n_without}</p>
        </div>
        <div className="rounded-xl bg-primary/5 p-2 text-center">
          ...
          {item.pct_change != null && (
            <p className="text-[8px] text-text-muted">{item.pct_change > 0 ? '+' : ''}{item.pct_change.toFixed(0)}%</p>
          )}
        </div>
      </div>
