...
export default function DeleteEventConfirmModal({ selectedEvent, deleting, onClose, executeDelete }: DeleteEventConfirmModalProps) {
  ...
  return (
    <Modal isOpen onClose={onClose} showCloseButton={false} size="xs">
      <p className="text-base font-black text-text-primary text-center">Usuń wydarzenie</p>
      ...
      {isRecurringInstance ? (
        <div className="space-y-2 pt-2">
          ...
          <Button
            ...
            disabled={deleting}
            className="w-full py-2.5 text-xs text-danger border-danger/40 hover:bg-danger/10"
          >
        ...
        <div className="flex gap-2.5 pt-2">
          <Button
            variant="secondary"
            onClick={onClose}
            className="flex-1 py-2.5 text-xs text-text-muted hover:text-text-primary"
          >
            Anuluj
          </Button>
          <Button
