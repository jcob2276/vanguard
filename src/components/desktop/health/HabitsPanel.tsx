  ...
  return (
    <Card padding="1rem 1.25rem" className="space-y-3">
      <div className="flex items-center justify-between">
        <div>
          <p className="text-2xs font-black uppercase tracking-[0.25em] text-text-muted">Nawyki</p>
      ...
      {isAddingHabit && (
        <Card variant="accent" padding="0.75rem" className="space-y-2">
          ...
          <div className="grid grid-cols-[44px_1fr] gap-2">
            <input value={newHabit.icon} onChange={e => setNewHabit(p => ({ ...p, icon: e.target.value }))} className="rounded-lg border border-border-custom bg-surface p-2 text-center text-sm font-black text-text-primary outline-none focus:border-primary/50" placeholder="✅" />
            <input value={newHabit.name} onChange={e => setNewHabit(p => ({ ...p, name: e.target.value }))} onKeyDown={e => e.key === 'Enter' && addHabit()} className="rounded-lg border border-border-custom bg-surface px-3 py-2 text-xs font-bold text-text-primary outline-none placeholder:text-text-muted/40 focus:border-primary/50" placeholder="Nazwa" />
          </div>
          <div className="grid grid-cols-2 gap-2">
            <button onClick={() => setNewHabit(p => ({ ...p, is_positive: true }))} className={`rounded-lg border py-2 text-2xs font-black uppercase tracking-widest cursor-pointer ${newHabit.is_positive ? 'border-success/35 bg-success/10 text-success' : 'border-border-custom text-text-muted'}`}>Wzmacniać</button>
            <button onClick={() => setNewHabit(p => ({ ...p, is_positive: false }))} className={`rounded-lg border py-2 text-2xs font-black uppercase tracking-widest cursor-pointer ${!newHabit.is_positive ? 'border-danger/35 bg-danger/10 text-danger' : 'border-border-custom text-text-muted'}`}>Unikać</button>
          </div>
