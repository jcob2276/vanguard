    ...
    <form onSubmit={onSubmit} className="space-y-3.5 transition-all">
      <div className="grid grid-cols-3 gap-3">
        <div className="col-span-2 space-y-1">
          ...
          <ControlInput type="text" required value={name} onChange={e => setName(e.target.value)} placeholder="np. Pyłek kwiatowy + Wit. C"
        ...
        <div className="space-y-1">
          ...
          <ControlInput type="text" maxLength={2} value={emoji} onChange={e => setEmoji(e.target.value)} placeholder="🐝"
      ...
      <div className="border-t border-border-custom/50 pt-3 space-y-2.5">
        <div className="flex items-center gap-2">
          <ControlInput type="checkbox" id="hasCycle" checked={hasCycle} onChange={e => setHasCycle(e.target.checked)}
            className="rounded border-border-custom bg-surface text-success focus:ring-0 cursor-pointer" />
        ...
        {hasCycle && (
          <div className="grid grid-cols-2 gap-3 pl-5 transition-all">
            ...
            <div className="space-y-1">
              ...
              <ControlInput type="date" required={hasCycle} value={endDate} onChange={e => setEndDate(e.target.value)}
                className="w-full rounded-lg border border-border-custom bg-surface px-3 py-1.5 text-xs focus:outline-none focus:border-success" />
            </div>
          </div>
        )}
      ...
      <div className="border-t border-border-custom/50 pt-3 space-y-2.5">
        <div className="flex items-center gap-2">
          <ControlInput type="checkbox" id="hasReminder" checked={hasReminder} onChange={e => setHasReminder(e.target.checked)}
            className="rounded border-border-custom bg-surface text-success focus:ring-0 cursor-pointer" />
      ...
      <Button type="submit" variant="primary" size="md" disabled={submitting} loading={submitting}
        className="w-full py-2.5 rounded-xl bg-success hover:bg-success-hover disabled:bg-success/40 disabled:text-text-muted text-on-accent text-xs font-black uppercase tracking-wider transition-colors cursor-pointer">
