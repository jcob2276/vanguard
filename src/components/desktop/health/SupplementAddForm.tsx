  ...
  return (
    ...
    <form onSubmit={onSubmit} className="space-y-3.5 transition-all">
      <div className="grid grid-cols-3 gap-3">
        <div className="col-span-2 space-y-1">
          ...
          <input type="text" required value={name} onChange={e => setName(e.target.value)} placeholder="np. Pyłek kwiatowy + Wit. C"
            className="w-full rounded-lg border border-border-custom bg-surface px-3 py-2 text-[11px] focus:outline-none focus:border-success" />
        </div>
          ...
          <div className="grid grid-cols-2 gap-3 pl-5 transition-all">
            <div className="space-y-1">
              ...
              <input type="date" required={hasCycle} value={startDate} onChange={e => setStartDate(e.target.value)}
                className="w-full rounded-lg border border-border-custom bg-surface px-3 py-1.5 text-[11px] focus:outline-none focus:border-success" />
            </div>
            <div className="space-y-1">
      ...
      <Button type="submit" variant="primary" size="md" disabled={submitting} loading={submitting}
        className="w-full py-2.5 rounded-xl bg-success hover:bg-success-hover disabled:bg-success/40 disabled:text-text-muted text-white text-[10px] font-black uppercase tracking-wider transition-colors cursor-pointer">
