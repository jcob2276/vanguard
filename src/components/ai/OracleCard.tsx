  ...
  return (
    <>
      {!open && !embedded ? (
        <Pressable
          onClick={handleOpen}
          style={{ transform: `scale(var(${btnPressed ? '--scale-pressed-strong' : '--scale-rest'}))`, transition: 'var(--ds-inline-style-transform-150ms-ease)' }}
        ...
        >
          ...
          <div>
            <p className="text-2xs font-black uppercase tracking-[var(--ds-arbitrary-0-2em)] text-primary/60">Oracle</p>
        ...
        <Card
          ...
          style={{
            animation: 'var(--ds-inline-style-oracle-slide-up-500ms-cubic-bezier-0-33-1-0-68-1-both)',
            transition: focused ? 'var(--motion-oracle-focus)' : 'var(--motion-oracle-idle)',
          }}
        >
          <style>{`
            @keyframes oracle-slide-up {
              from { opacity:var(--ds-inline-css-0-coll-4); transform: translateY(24px) scale(0.97); }
              to   { opacity:var(--ds-inline-css-1); transform: translateY(0) scale(1); }
            }
          `}</style>
          ...
          <div className="flex items-center justify-between px-5 pt-4 pb-3 border-b border-border-custom">
            ...
            {!embedded && (
              <Pressable
                ...
                className="rounded-full p-2.5 min-h-[var(--ds-h-44px)] min-w-[var(--ds-w-44px)] flex items-center justify-center text-text-muted hover:bg-surface-solid hover:text-text-primary transition-all cursor-pointer"
              >
                <X size={14} />
              </Pressable>
            )}
