      ...
      <ol className="space-y-2">
        {data.steps.map(s => (
          <li key={s.step} className="flex items-start gap-3">
            <span className="w-5 h-5 rounded-full flex-shrink-0 flex items-center justify-center text-xs font-bold" style={{ background: s.done ? 'var(--color-theme-hex-ba16185129015)' : 'var(--color-theme-hex-ba9110825501)', color: s.done ? 'var(--color-success)' : 'var(--color-primary)' }}>{s.step}</span>
