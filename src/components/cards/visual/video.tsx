...
export function VideoCard({ data }: { data: VideoData }) {
  return (
    <div>
      <div className="relative rounded-xl overflow-hidden aspect-video bg-black flex items-center justify-center cursor-pointer" onClick={() => data.url && window.open(data.url, '_blank')}>
        ...
        <div className="relative z-10 w-12 h-12 rounded-full bg-white/90 flex items-center justify-center shadow-lg">
          <Play size={18} style={{ color: '#0A0A0A' }} />
        </div>
        {data.duration && <span className="absolute bottom-2 right-2 text-[10px] font-bold text-white bg-black/60 px-1.5 py-0.5 rounded">{data.duration}</span>}
      </div>
