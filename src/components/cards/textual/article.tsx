...
export function ArticleCard({ data }: { data: ArticleData }) {
  return (
    <div className="space-y-2">
      <p className="text-lg font-bold leading-snug" style={{ color: 'var(--text-primary)' }}>{data.title}</p>
      ...
      <p className="text-sm leading-relaxed" style={{ color: 'var(--text-secondary)' }}>{data.body}</p>
    </div>
  );
}
