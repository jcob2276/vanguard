    ...
    <div className="space-y-2">
      ...
      {data.messages.map((m, i) => (
        <div key={i} className={`flex ${m.isUser ? 'flex-row-reverse' : 'flex-row'} gap-2`}>
          <div className="w-5 h-5 rounded-full flex-shrink-0 flex items-center justify-center text-2xs font-bold" style={{ background: m.isUser ? 'var(--color-primary)' : 'var(--legacy-color-088)', color: m.isUser ? 'var(--color-on-accent)' : 'var(--color-text-tertiary)' }}>{m.speaker[0]}</div>
