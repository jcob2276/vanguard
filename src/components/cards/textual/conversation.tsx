    ...
    <div className="space-y-2">
      ...
      {data.messages.map((m, i) => (
        <div key={i} className={`flex ${m.isUser ? 'flex-row-reverse' : 'flex-row'} gap-2`}>
          <div className="w-5 h-5 rounded-full flex-shrink-0 flex items-center justify-center text-2xs font-bold" style={{ background: m.isUser ? 'var(--color-primary)' : 'rgba(153,161,175,0.2)', color: m.isUser ? 'white' : 'var(--color-text-tertiary)' }}>{m.speaker[0]}</div>
          <div className="max-w-[80%]">
            <p className="text-xs font-medium mb-0.5" style={{ color: 'var(--color-text-tertiary)' }}>{m.speaker}</p>
            <p className="text-sm rounded-xl px-2.5 py-1.5" style={{ background: m.isUser ? 'rgba(91,108,255,0.08)' : 'rgba(153,161,175,0.08)', color: 'var(--text-secondary)' }}>{m.text}</p>
          </div>
        </div>
      ))}
    </div>
