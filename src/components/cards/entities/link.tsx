...
export function LinkCard({ data }: { data: LinkCardData }) {
  return (
    <a href={data.url} target="_blank" rel="noopener noreferrer" className="flex items-start gap-3 no-underline">
      <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 bg-[rgba(91,108,255,0.08)]">
        {data.favicon ? <img src={data.favicon} className="w-4 h-4 rounded" alt="" /> : <ExternalLink size={13} style={{ color: '#5B6CFF' }} />}
      </div>
