...
export default function ProjectEvidenceStrip({ userId, projectId }: { userId: string; projectId: string }) {
  ...
  if (items.length === 0) {
    return (
      <p className="text-xs text-text-muted/60 mt-3 border-t border-border-custom/50 pt-3">
  ...
  return (
    <div className="mt-3 border-t border-border-custom/50 pt-3 space-y-1.5">
      <p className="text-2xs font-black uppercase tracking-wider text-text-muted">Dowód · 7 dni</p>
      {items.map((item, i) => (
        <div key={`${item.kind}-${item.date}-${i}`} className="flex items-start gap-2 text-xs">
          <span className="shrink-0 font-black text-text-muted tabular-nums">{item.date.slice(5)}</span>
          <span className="shrink-0 rounded px-1 py-0.5 text-3xs font-black uppercase bg-primary/10 text-primary">
