...
export function FocusProjectBanner({ focusProject, activeFilteredFirst, stats, kpisByProject, onOpen }: Props) {
  ...
  return (
    <Card variant="outline" padding="1rem" className={hc.bg} style={{ borderColor: level === 'critical' ? 'rgba(239,68,68,0.3)' : 'rgba(245,158,11,0.25)' }}>
      ...
      <p className="text-base font-bold text-text-primary">{focusProject.name}</p>
      {focusProject.goal && (
        <p className="text-xs text-text-muted mt-0.5 italic line-clamp-1">{focusProject.goal}</p>
      )}
      <div className="mt-3 flex items-center gap-2">
        <Button
          ...
          className={`text-xs px-3 py-1.5 rounded-full ${hc.text} ${
        ...
        {s.openItems?.[0] && (
          <p className="text-xs text-text-muted truncate">→ {s.openItems[0].title}</p>
        )}
      </div>
    </Card>
  );
