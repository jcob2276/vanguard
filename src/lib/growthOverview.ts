...
export function buildGrowthSuggestions(input: {
  hasDirectionGoals: boolean;
  ...
  focus: FocusProposal | null;
  sprintGoal: string | null;
}): GrowthSuggestion[] {
  const out: GrowthSuggestion[] = [];

  if (!input.hasDirectionGoals && !input.sprintGoal) {
    out.push({
      id: 'direction',
      tone: 'action',
      title: 'Brak kontekstu tygodnia',
      body: 'Ustaw intencję w Tydzień albo cel sprintu w Projekty — wtedy focus ma sens.',
      view: 'tydzien',
    });
  }
  ...
  if (input.activeProjectCount === 0) {
    out.push({
      ...
      body: 'Rozwój skilli bez projektu to teoria w próżni. Aktywuj projekt z KPI.',
      view: 'projekty',
    });
  } else if (input.kpiBehind && input.kpiLabel) {
    out.push({
      id: 'kpi',
      tone: 'action',
      title: `KPI: ${input.kpiLabel}`,
      body: 'Dowód biznesowy jest za deklaracją skilli — dociągnij KPI w Projekty.',
      view: 'projekty',
    });
  }

  if (input.focus) {
    const weak = input.focus.subskillLabel ?? input.focus.parentLabel;
    out.push({
      id: 'focus',
      tone: 'neutral',
      title: `Propozycja focusu: ${weak}`,
      body:
        input.focus.source === 'saved'
          ? `Zapisany focus · ocena ${input.focus.score}/5. Egzekucja w Todo / PowerList.`
          : `Najsłabsze ogniwo (${input.focus.score}/5). Działaj w Todo, mierz w Projekty.`,
      view: 'todo',
    });
  }
  ...
  if (input.mustOpen > 0) {
    out.push({
      id: 'must',
      tone: 'warn',
      title: `${input.mustOpen} otwartych MUST`,
      body: 'Dokończ lub odpiń w Todo — stary plan blokuje widok tygodnia.',
      view: 'todo',
    });
  }

  if (input.practiceShare > 0 && input.practiceShare < 50) {
    out.push({
      id: 'theory',
      tone: 'warn',
      title: 'Dużo teorii, mało repów',
      body: 'Zamień kolejny link na zadanie w Todo albo wygrane w PowerList.',
      view: 'keep',
    });
  }

  if (input.unreadLinks >= 5) {
    out.push({
      id: 'keep',
      tone: 'neutral',
      title: `${input.unreadLinks} linków w Keep`,
      body: 'Skarbnica rośnie — wybierz max 1–2 na tydzień, reszta czeka.',
      view: 'keep',
    });
  }

  if (input.openTodos === 0 && input.activeProjectCount > 0) {
    out.push({
      id: 'todo-empty',
      tone: 'action',
      title: 'Brak otwartych zadań',
      body: 'Skill bez zadania to deklaracja. Dodaj rep w Todo powiązany z projektem.',
      view: 'todo',
    });
  }
