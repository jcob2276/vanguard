...
describe('deriveSpineGuidance', () => {
  ...
  it('prompts KPI review when overdue', () => {
    const g = deriveSpineGuidance(baseSpine(), { ...readyCtx(), kpiOverdueDays: 14 });
    expect(g.primaryAction).toMatchObject({ type: 'navigate', target: 'weekly-review' });
  });
