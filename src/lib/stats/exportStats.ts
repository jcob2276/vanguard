...
interface ExportStatsMarkdownParams {
...
export async function exportStatsMarkdown({
  ...
  includeActivityWatch,
    ...
    const parseLocalDateToIso = (dateStr: string, timeStr: string): string => {
    ...
    const toWarsawTime = (value: string | number | Date) => new Date(value).toLocaleTimeString('pl-PL', { timeZone: 'Europe/Warsaw', hour: '2-digit', minute: '2-digit' });
      ...
      if (includeNutrition) {
        ...
        if (dayFood.length > 0) {
          ...
          if (loggedTimes.length >= 2) {
            ...
            const fmtT = (d: Date) => d.toLocaleTimeString('pl-PL', { timeZone: 'Europe/Warsaw', hour: '2-digit', minute: '2-digit' });
