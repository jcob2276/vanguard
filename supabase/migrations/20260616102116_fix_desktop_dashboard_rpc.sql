-- Fix desktop dashboard RPC after frontend switched to the consolidated loader.
-- The previous version compared timestamptz columns to text and still read the
-- deprecated career_* module. Keep the JSON contract stable for the UI.
...
BEGIN
  ...
  SELECT jsonb_build_object(
    'oura', (
      ...
      FROM (
        SELECT date, hrv_avg, rhr_avg, total_sleep_hours, readiness_score
    ...
    'sessions', (
      ...
      FROM (
        ...
        WHERE ws.user_id = p_user_id AND ws.date >= (v_today - 91)
      ...
      FROM (
        SELECT
          ...
          CASE
            ...
            WHEN p.status = 'paused' THEN 'unsure'
            ELSE 'sense'
          END AS sense_status,
          NULL::text AS area,
          p.goal AS thesis
        FROM public.projects p
        WHERE p.user_id = p_user_id AND p.status <> 'done'
    ...
    'moves', (
      ...
      FROM (
        SELECT
          ...
          ti.title,
          CASE
            WHEN ti.status = 'open' THEN 'todo'
            WHEN ti.status = 'done' THEN 'done'
            ELSE ti.status
          END AS status,
          ti.completed_at,
          ti.due_date AS planned_for,
          ts.project_id
        FROM public.todo_items ti
        JOIN public.todo_sections ts ON ts.id = ti.section_id
        WHERE ti.user_id = p_user_id
          AND ts.project_id IS NOT NULL
          AND ti.status <> 'dropped'
    ...
    'habitLogs', (
      ...
      FROM (
        ...
        WHERE user_id = p_user_id AND date >= (v_today - 30)
      ) sub
    )
  ) INTO v_result;
