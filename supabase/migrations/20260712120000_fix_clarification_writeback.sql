-- ============================================================
-- VANGUARD OS — FIX §1.1: CLARIFICATION WRITEBACK TRIGGER
-- ============================================================
-- Fixes two bugs in handle_clarification_writeback():
--
-- Bug 1: trigger only checked `option_ids ? 'yes'` — never fired for
--        single_choice / multi_choice responses (where option_ids holds
--        arbitrary ids like 'opt1', 'a', etc.).
--
-- Bug 2: INSERT into vanguard_entity_links (which has a BEFORE trigger
--        enforcing vanguard_relation_ontology) was NOT wrapped in an
--        EXCEPTION block → any LLM-generated relation outside the
--        ontology caused SQLERRM to propagate and roll back the entire
--        UPDATE on oracle_clarification_requests (user got a 500 error
--        and their answer was not saved).
-- ============================================================
...
BEGIN
  ...
  --   single_choice/multi_choice → option_ids is non-empty and does NOT contain '__uncertain__'
  ...
  EXCEPTION WHEN OTHERS THEN
    ...
    -- (old trigger fell back to relation='fakt' which is outside ontology and crashed)
  ...
  EXCEPTION WHEN OTHERS THEN
    ...
    -- The answer is still saved; memory writeback is best-effort.
...
-- Re-attach trigger (DROP + CREATE to pick up the new function body)
