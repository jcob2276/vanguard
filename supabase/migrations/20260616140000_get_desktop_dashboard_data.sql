-- Consolidated dashboard data RPC for Vanguard Desktop
  ...
  SELECT jsonb_build_object(
    ...
    'strava', (
      ...
      FROM (
        ...
        WHERE user_id = p_user_id AND start_date >= ((v_today - 84)::text || 'T00:00:00')
    ...
    'projects', (
      SELECT COALESCE(jsonb_agg(sub), '[]'::jsonb)
      FROM (
        SELECT id, name, status, sense_status, area, thesis
    ...
    'moves', (
      SELECT COALESCE(jsonb_agg(sub), '[]'::jsonb)
      FROM (
        SELECT id, title, status, completed_at, planned_for, project_id
        FROM public.career_moves
        WHERE user_id = p_user_id AND status <> 'dropped'
    ...
    'stream', (
      ...
      FROM (
        ...
        WHERE user_id = p_user_id 
          ...
          AND timestamp >= ((v_today - 14)::text || 'T00:00:00')
