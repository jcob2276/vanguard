-- Fix save_workout_atomic:
-- 1. rpe no longer falls back to rir (was: COALESCE(rpe, rir))
-- 2. is_pws_or_msp now written to exercise_logs
-- 3. importance_score computed and updated on workout_sessions
...
BEGIN
  ...
  VALUES (
    p_user_id,
    p_day_key,
    COALESCE((p_start_time AT TIME ZONE 'Europe/Warsaw')::date, CURRENT_DATE),
    p_start_time,
    p_end_time,
  ...
  -- Compute importance_score: base 5, adjusted by volume / msp / effort
  SELECT
    COUNT(*),
    AVG(NULLIF(el->>'rir', '')::decimal)
