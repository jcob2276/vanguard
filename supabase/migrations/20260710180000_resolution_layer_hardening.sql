-- ============================================================
-- VANGUARD OS — RESOLUTION LAYER HARDENING (P2.2)
-- ============================================================
...
BEGIN
    ...
    -- P2.2: Log weekly report of entity_fuzzy_ambiguous to audit_events
    SELECT count(*) INTO v_ambiguous_count
    FROM public.audit_events
    WHERE event_type = 'entity_fuzzy_ambiguous'
      AND created_at > now() - interval '7 days';

    IF v_ambiguous_count > 0 THEN
        INSERT INTO public.audit_events (event_type, severity, message, metadata)
        VALUES (
            'entity_resolution_weekly_report',
            'info',
            'Weekly Entity Resolution Report: ' || v_ambiguous_count || ' ambiguous fuzzy matches occurred in the last 7 days.',
            jsonb_build_object('ambiguous_count', v_ambiguous_count)
        );
    END IF;
END;
$$;
