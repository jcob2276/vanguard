...
Deno.serve(serveJson(async (req, ctx) => {
  ...
  const { date, dateFrom, dateTo } = body
  ...
  if (fastingLog) return { success: true, mode: 'single', date: targetDate, fasting: true, day_quality_score: null, day_quality_analysis: fastingLog.note || 'Dzień świadomego postu.', items: [] }
  ...
  if (!todayEntries || todayEntries.length === 0) throw new Error('Brak wpisów żywieniowych dla tego dnia')
