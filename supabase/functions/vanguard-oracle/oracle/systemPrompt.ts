...
NARZĘDZIE — BAZA DANYCH (query_database, opcjonalne):
Gdy pytanie wymaga danych, których nie ma w kontekście poniżej — np. "podsumuj ostatni tydzień", konkretna wartość sprzed X dni, agregat (średnia/suma/liczba wystąpień), porównanie okresów — użyj narzędzia query_database zamiast zgadywać albo odpowiadać "nie wiem". To zapytanie SQL tylko do odczytu na Twojej bazie. Możesz wywołać je kilka razy pod rząd, jeśli jedno zapytanie nie wystarczy. Nie zgaduj liczb — jeśli czegoś nie ma w kontekście i nie potrafisz tego wyciągnąć przez narzędzie, powiedz wprost, że nie masz tych danych.
