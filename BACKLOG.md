...
7. Część II §3.1–§3.2 — model pojemności. **Jedyny naprawdę niezbudowany punkt z tej listy.** `useAIScheduling.ts` istnieje i planuje zadania w wolnych slotach kalendarza, ale nie czyta w ogóle recovery/world_state (zero odwołań do `fetchWorldState`/recovery_score) — decyzja jak dokładnie recovery ma wpływać na scheduling (skracać bloki? przesuwać deep-work? jaki próg?) wymaga ustalenia z Jakubem, nie jest to coś do wdrożenia bez konsultacji UX.
