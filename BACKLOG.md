...
4. [x] (częściowo) Offline write queue — **znacznie szerszy niż zapisano tutaj**: `lib/offlineQueue.ts` (`queueOfflineWrite`/`rpcWithOfflineFallback`) już pokrywa notatki (`useNotesMutations.ts`), todo (`todo.ts`), nawyki (`habitsApi.ts`), suplementy (`supplementsApi.ts`), trening (`workoutLogging.ts`) i posiłki (`useFoodEntryActions.ts`) — 6 domen, nie 2. Prawdziwa luka: `dreamsApi.ts`, `calendarApi.ts`, `kpiTrendApi.ts` mają zero pokrycia offline. Architektura "react-query persist + mutation queue" (osobna biblioteka `@tanstack/react-query-persist-client`) nigdy nie zainstalowana — obecny ad-hoc wzorzec działa, ale to inna, większa decyzja architektoniczna niż rozszerzenie istniejącego `queueOfflineWrite` na 3 brakujące domeny.
5. [x] Skróty PWA — **już zrobione**: `vite.config.ts` ma pełny `manifest.shortcuts` (`/keep?new=1`, `/?todo=new`, `/?meal=new`, `/trening`) + `share_target`, a `useDashboardState.ts:53,188` faktycznie czyta te query paramy i reaguje. Ten punkt backlogu był nieaktualny.
6. [x] Miniatury client-side — **już zrobione**: `lib/imageThumbnail.ts` (`generateThumbnail`) używane w `Photos.tsx:88` przy uploadzie. Ten punkt backlogu był nieaktualny.
7. [x] Część II §3.1–§3.2 — model pojemności. **Decyzja 2026-07-12: usunięte, nie budowane.** Jakub nie używał przycisku "Zaplanuj AI" i nie planuje — funkcja szła wbrew zasadzie zero-maintenance (wymagałaby effortu na zadaniu, którego nikt nie wpisuje) i jedyna uczciwa forma integracji z recovery (cichy wpływ na algorytm, nie ostrzeżenie — reguła "zero alertów prewencyjnych") nie miała odbiorcy. Usunięto `useAIScheduling.ts`, przycisk "AI" w `CalendarHeader.tsx`, martwy stan `focusTimeDefense`/`decompressionBuffer` (setter nigdy nie wołany) z `useCalendarData.ts`, i cały pass-through w `useCalendarIntegrations.ts`/`CalendarContext.tsx`.
